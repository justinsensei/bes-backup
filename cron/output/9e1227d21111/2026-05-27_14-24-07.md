...
stderr:
ERROR: poll_bes_inbox: gmail search failed (rc=1): Token is invalid. Re-run setup.
```
...
Gmail token is invalid. I can't check the inbox.

Run this to re-authenticate:
```bash
python ~/bes-backup/skills/productivity/google-workspace/scripts/setup.py --auth-url
```
