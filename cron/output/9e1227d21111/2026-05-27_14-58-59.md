...
The email poller failed because the Gmail OAuth token for `personal-main` has expired or been revoked (`invalid_grant`).

To fix this, re-authenticate by running:

```bash
python3 ~/.hermes/skills/productivity/google-workspace/scripts/setup.py --revoke
```

Follow the prompts to authorize a new token.
