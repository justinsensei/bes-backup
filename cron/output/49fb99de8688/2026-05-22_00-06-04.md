...
Work log written to `2026-05-21 Thursday.md`. Here's what went in:

---

**Thursday May 21 work log — done.**

Busy day. Key things captured:

**Highlights:** ASL Bloom App Store feature, big \#product-leads streak-screen thread (landed on Day-0-free variant test), fixed a Customer.io campaign outage (NULL≠false bug with Parag/Endre), RevenueCat events migrated to PostHog SDK, Clio sign_language coverage audit + two Linear tickets filed (PROD-532, PROD-533), Mobile Game Doctors kickoff + meeting scheduled, two school tasks handled (Rosa's ADHD screener sent, Sam's PPS ID retrieved).

**Decisions:** Streak screens moving D0→D1 via streak-free variant; NULL≠false flagged as structural data risk (PROD-532); RevenueCat routing fixed.

**Open items:** Tuition invoice discrepancy (P1 Todoist), Google Play compliance flag on MeLISegno, revenue reporting gap (Clio hit a dead end), Kristina still looking into Freshdesk default-assign, Viren track change needs an owner, Fidelity 401K rollover decision pending.
